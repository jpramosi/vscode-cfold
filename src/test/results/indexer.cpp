...
struct IndexParam { @_3_
  ...
  IndexFile *consumeFile(FileID fid) { @_6_
    seenFile(fid);
    return uid2file[fid].db.get();
  } @_6_

  bool useMultiVersion(FileID fid) { @_7_
...
Kind getKind(const Decl *d, SymbolKind &kind) { @_9_
  switch (d->getKind()) {
...
LanguageId getDeclLanguage(const Decl *d) { @_10_
  switch (d->getKind()) {
  default:
    return LanguageId::C;
  case Decl::ImplicitParam:
...
QualType getBaseType(QualType t, bool deduce_auto) { @_11_
  QualType baseType = t;
...
const Decl *getTypeDecl(QualType t, bool *specialization = nullptr) { @_12_
  Decl *d = nullptr;
...
const Decl *getAdjustedDecl(const Decl *d) { @_13_
  while (d) {
...
bool validateRecord(const RecordDecl *rd) { @_14_
...
class IndexDataConsumer : public index::IndexDataConsumer { @_15_
public:
  ASTContext *ctx;
  IndexParam &param;

  std::string getComment(const Decl *d) { @_16_
  ...
  Usr getUsr(const Decl *d, IndexParam::DeclInfo **info = nullptr) const { @_17_
  ...
  PrintingPolicy getDefaultPolicy() const { @_18_
  ...
  static void simplifyAnonymous(std::string &name) { @_19_
  ...
  void setName(const Decl *d, std::string_view short_name,
               std::string_view qualified, Def &def) { @_20_
    SmallString<256> str;
  ...
  void setVarName(const Decl *d, std::string_view short_name,
                  std::string_view qualified, IndexVar::Def &def) { @_21_
    QualType t;
  ...
  static int getFileLID(IndexFile *db, SourceManager &sm, FileID fid) { @_22_
  ...
  void collectRecordMembers(IndexType &type, const RecordDecl *rd) { @_24_
...
#if LLVM_VERSION_MAJOR < 10 // llvmorg-10-init-12036-g3b9715cb219 @_25_
# define handleDeclOccurrence handleDeclOccurence
#endif @_25_
  bool handleDeclOccurrence(const Decl *d, index::SymbolRoleSet roles,
                            ArrayRef<index::SymbolRelation> relations,
                            SourceLocation src_loc,
                            ASTNodeInfo ast_node) override { @_26_
    if (!param.no_linkage) {
...
class IndexPPCallbacks : public PPCallbacks { @_28_
  SourceManager &sm;
  IndexParam &param;

  std::pair<StringRef, Usr> getMacro(const Token &tok) const { @_29_
...
public:
  ...
  void FileChanged(SourceLocation sl, FileChangeReason reason,
                   SrcMgr::CharacteristicKind, FileID) override { @_30_
  ...
  void InclusionDirective(SourceLocation hashLoc, const Token &tok,
                          ...
                          SrcMgr::CharacteristicKind fileType) override { @_31_
    if (!file)
      return;
  ...
  void MacroDefined(const Token &tok, const MacroDirective *md) override { @_32_
  ...
  void MacroUndefined(const Token &tok, const MacroDefinition &md,
                      const MacroDirective *ud) override { @_34_
    if (ud) {
      SourceLocation sl = ud->getLocation();
      MacroExpands(tok, md, {sl, sl}, nullptr);
    }
  } @_34_
  void SourceRangeSkipped(SourceRange sr, SourceLocation) override { @_35_
...
class IndexFrontendAction : public ASTFrontendAction { @_36_
...
public:
  ...
  std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &ci,
                                                 StringRef inFile) override { @_37_
...
#if LLVM_VERSION_MAJOR >= 10 // rC370337 @_38_
...
IndexFunc &IndexFile::toFunc(Usr usr) { @_39_
...
IndexType &IndexFile::toType(Usr usr) { @_40_
...
IndexVar &IndexFile::toVar(Usr usr) { @_41_
...
std::string IndexFile::toString() { @_42_
  return ccls::serialize(SerializeFormat::Json, *this);
} @_42_

template <typename T> void uniquify(std::vector<T> &a) { @_43_
...
#if LLVM_VERSION_MAJOR >= 9 // rC357037 @_47_
...
#if LLVM_VERSION_MAJOR >= 10 // rC370337 @_50_
  ...
  std::string reason;
  {
    ...
    auto parse = [&]() {
      if (!action->BeginSourceFile(*clang, clang->getFrontendOpts().Inputs[0]))
        return;
#if LLVM_VERSION_MAJOR >= 9 // rL364464 @_52_
...
void reflect(JsonReader &vis, SymbolRef &v) { @_54_
...
void reflect(JsonReader &vis, Use &v) { @_55_
  std::string t = vis.getString();
  char *s = const_cast<char *>(t.c_str());
  v.range = Range::fromString(s);
  s = strchr(s, '|');
...
void reflect(JsonReader &vis, DeclRef &v) { @_56_
  std::string t = vis.getString();
  char *s = const_cast<char *>(t.c_str());
  v.range = Range::fromString(s);
  s = strchr(s, '|') + 1;
...
void reflect(JsonWriter &vis, SymbolRef &v) { @_57_
  char buf[99];
...
void reflect(JsonWriter &vis, Use &v) { @_58_
  char buf[99];
...
void reflect(JsonWriter &vis, DeclRef &v) { @_59_
  char buf[99];
...
void reflect(BinaryReader &vis, SymbolRef &v) { @_60_
  reflect(vis, v.range);
  reflect(vis, v.usr);
  reflect(vis, v.kind);
  reflect(vis, v.role);
} @_60_
void reflect(BinaryReader &vis, Use &v) { @_61_
  reflect(vis, v.range);
  reflect(vis, v.role);
  reflect(vis, v.file_id);
} @_61_
void reflect(BinaryReader &vis, DeclRef &v) { @_62_
  reflect(vis, static_cast<Use &>(v));
  reflect(vis, v.extent);
} @_62_

void reflect(BinaryWriter &vis, SymbolRef &v) { @_63_
  reflect(vis, v.range);
  reflect(vis, v.usr);
  reflect(vis, v.kind);
  reflect(vis, v.role);
} @_63_
void reflect(BinaryWriter &vis, Use &v) { @_64_
  reflect(vis, v.range);
  reflect(vis, v.role);
  reflect(vis, v.file_id);
} @_64_
void reflect(BinaryWriter &vis, DeclRef &v) { @_65_
  reflect(vis, static_cast<Use &>(v));
  reflect(vis, v.extent);
} @_65_
} // namespace ccls @_0_
