...
bool cacheInvalid(VFS *vfs, IndexFile *prev, const std::string &path,
                  ...
                  const std::optional<std::string> &from) { @_11_
  {
...
std::string appendSerializationFormat(const std::string &base) { @_12_
...
std::string getCachePath(std::string src) { @_13_
...
std::unique_ptr<IndexFile> rawCacheLoad(const std::string &path) { @_15_
...
std::mutex &getFileMutex(const std::string &path) { @_16_
...
bool indexer_Parse(SemaManager *completion, WorkingFiles *wfiles,
                   Project *project, VFS *vfs, const GroupMatch &matcher) { @_17_
...
void quit(SemaManager &manager) { @_18_
...
void threadEnter() { @_19_
  std::lock_guard lock(thread_mtx);
  active_threads++;
} @_19_

void threadLeave() { @_20_
  std::lock_guard lock(thread_mtx);
  if (!--active_threads)
...
void main_OnIndexed(DB *db, WorkingFiles *wfiles, IndexUpdate *update) { @_23_
  if (update->refresh) {
    LOG_S(INFO)
...
void standalone(const std::string &root) { @_27_
  Project project;
  WorkingFiles wfiles;
  VFS vfs;
  SemaManager manager(
...
std::optional<std::string> loadIndexedContent(const std::string &path) { @_30_
  if (g_config->cache.directory.empty()) {
    std::shared_lock lock(g_index_mutex);
    auto it = g_index.find(path);
    if (it == g_index.end())
...
void notifyOrRequest(const char *method, bool request,
                     const std::function<void(JsonWriter &)> &fn) { @_31_
...
static void reply(const RequestId &id, const char *key,
                  const std::function<void(JsonWriter &)> &fn) { @_32_
  rapidjson::StringBuffer output;
  rapidjson::Writer<rapidjson::StringBuffer> w(output);
  w.StartObject();
  w.Key("jsonrpc");
...
void reply(const RequestId &id, const std::function<void(JsonWriter &)> &fn) { @_33_
  reply(id, "result", fn);
} @_33_

void replyError(const RequestId &id,
                const std::function<void(JsonWriter &)> &fn) { @_34_
  reply(id, "error", fn);
} @_34_
} // namespace pipeline @_7_
} // namespace ccls @_1_
